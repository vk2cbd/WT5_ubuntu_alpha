# R-I Ubuntu Alpha Requirements

R-I Ubuntu Alpha is a clean rebuild of the WT4 prototype antenna controller. WT4 should remain
available as the reference implementation and field-tested prototype. R-I Ubuntu Alpha should
...
R-I Ubuntu Alpha should be split into clear modules. Suggested structure:

- `ri_protocol.py`: serial protocol only.
- `ri_safety.py`: limits, azimuth dead-zone logic, path choice, safety checks.
- `ri_antenna.py`: one antenna session, position reads, OLED writes, guarded
  moves.
- `ri_tracking.py`: Sun, Moon, and source tracking logic.
- `ri_power.py`: RTL-SDR power meter and RTL calibration.
- `ri_calibration.py`: manual calibration, peak calibration, scan calibration,
  Gaussian fitting, Y factor.
- `ri_config.py`: INI configuration load/save and defaults.
- `ri_logging.py`: event and measurement logging.
- `ri_gui.py`: GUI presentation and user interaction only.
...
Long slews should not be opaque single calls. R-I Ubuntu Alpha should prefer a segmented
...
R-I Ubuntu Alpha should detect and log:
...
R-I Ubuntu Alpha must make East/West differences visible.
...
The R-I Ubuntu Alpha GUI should be more aesthetically pleasing while remaining practical and
field-friendly.
...
R-I Ubuntu Alpha needs both event logging and measurement logging.
...
R-I Ubuntu Alpha should include a simulation backend before hardware tests.
...
1. Create R-I Ubuntu Alpha skeleton and this requirements document.
...
16. Field-test against WT4 behaviours before relying on R-I Ubuntu Alpha operationally.

## Non-Goals For Initial R-I Ubuntu Alpha
...
- Do not remove WT4 until R-I Ubuntu Alpha has proven field behaviour.

## Open Questions

- Should R-I Ubuntu Alpha support both RTL-SDR and SDRplay in the first clean rebuild, or
  defer SDRplay until R-I Ubuntu Alpha is stable?
...
- Should R-I Ubuntu Alpha store measurement metadata in a small SQLite database later, while
  still exporting CSV?
